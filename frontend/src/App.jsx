...
export default function App() {
  ...
  async function handleScan() {
    ...
    try {
      ...
      setError('Backend error. Make sure the server is running on port 8000.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div style={{ minHeight: '100vh', background: '#0f0f13', fontFamily: 'Inter, -apple-system, sans-serif' }}>
      ...
      <main style={{ maxWidth: 960, margin: '0 auto', padding: '28px 16px' }}>
        ...
        <div style={{
          ...
          border: '1px solid #2a2a3e', padding: 24, marginBottom: 20,
        }}>
          ...
          <div
            ...
            style={{
              border: `2px dashed ${preview ? '#6366f1' : '#3a3a50'}`,
              borderRadius: 10, padding: 20, textAlign: 'center',
              cursor: 'pointer', marginBottom: 14,
              background: preview ? '#12122a' : 'transparent',
            }}
          >
          ...
          <textarea
            ...
            placeholder="Or paste caption / message text here..."
            rows={3}
            style={{
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
            <button
              ...
              style={{
                ...
                background: loading ? '#3a3a50' : 'linear-gradient(135deg, #6366f1, #8b5cf6)',
        ...
        {result && risk && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {/* ROW 1 — Score + Risk + Confidence breakdown */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>

              {/* Score card */}
              <div style={{
                background: '#1a1a24', borderRadius: 14,
                border: `1px solid ${risk.color}40`, padding: 20,
              }}>
                  ...
                  <div>
                    ...
                    <div>
                      <span style={{
                        ...
                        background: '#1e1e30', border: '1px solid #3a3a50',
                        fontSize: 11, color: '#a0a0c0', textTransform: 'uppercase', letterSpacing: 1,
                      }}>
              ...
              <div style={{
                background: '#1a1a24', borderRadius: 14,
                border: '1px solid #2a2a3e', padding: 20,
              }}>
                <p style={{ fontSize: 11, color: '#6b7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 14 }}>
                    ...
                    <div key={layer} style={{ marginBottom: 10 }}>
                      ...
                      <div style={{ height: 5, background: '#2a2a3e', borderRadius: 3, overflow: 'hidden' }}>
                        <div style={{
                          ...
                          background: LAYER_COLORS[layer] || '#6366f1', borderRadius: 3,
                        }} />
                      </div>
                    </div>
                  ))
                ...
                <div style={{
                  borderTop: '1px solid #2a2a3e', paddingTop: 10, marginTop: 4,
            ...
            {/* ROW 2 — Weighted Evidence */}
            {result.weighted_evidence?.length > 0 && (
              <div style={{
                background: '#1a1a24', borderRadius: 14,
                border: `1px solid ${risk.color}30`, padding: 20,
              }}>
                <p style={{ fontSize: 11, color: '#6b7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 14 }}>
                  🔎 Evidence Detected
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {result.weighted_evidence.slice(0, 7).map((e, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{
                        minWidth: 160, fontSize: 12, color: risk.color,
                      ...
                      <div style={{ flex: 1, height: 5, background: '#2a2a3e', borderRadius: 3, overflow: 'hidden' }}>
                        <div style={{
                          width: `${Math.min(100, e.weight * 4)}%`, height: '100%',
                          background: risk.color, borderRadius: 3, opacity: 0.8,
                        }} />
                      </div>
            ...
            {result.url_analysis?.length > 0 && (
              <div style={{
                ...
                border: '1px solid #ef444440', padding: 20,
              }}>
                <p style={{ fontSize: 11, color: '#ef4444', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 14 }}>
                  🔗 Domain Intelligence
                </p>
                {result.url_analysis.map((ua, i) => (
                  <div key={i} style={{
                    background: '#12121e', borderRadius: 10,
                    border: '1px solid #2a2a3e', padding: '14px 16px',
                    marginBottom: i < result.url_analysis.length - 1 ? 10 : 0,
                  }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                      <div>
                        <p style={{ fontSize: 13, color: '#fca5a5', fontFamily: 'monospace', margin: '0 0 4px' }}>
                          {ua.domain}
                        </p>
                        {ua.impersonated_brand && (
                          <span style={{
                            padding: '2px 8px', borderRadius: 12,
                            background: '#f59e0b20', border: '1px solid #f59e0b40',
                            fontSize: 11, color: '#fcd34d',
                          }}>⚠ Impersonates {ua.impersonated_brand}</span>
                        )}
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{
                          fontSize: 20, fontWeight: 700,
                          color: ua.risk_score >= 70 ? '#ef4444' : ua.risk_score >= 45 ? '#f59e0b' : '#22c55e',
                        }}>{ua.risk_score}%</div>
                        <div style={{
                        ...
                        <div style={{
                          ...
                          background: ua.verdict === 'MALICIOUS' ? '#ef444420' : ua.verdict === 'SUSPICIOUS' ? '#f59e0b20' : '#22c55e20',
                          color: ua.verdict === 'MALICIOUS' ? '#ef4444' : ua.verdict === 'SUSPICIOUS' ? '#f59e0b' : '#22c55e',
                          border: `1px solid ${ua.verdict === 'MALICIOUS' ? '#ef444440' : ua.verdict === 'SUSPICIOUS' ? '#f59e0b40' : '#22c55e40'}`,
                        }}>⚠ Suspicious Domain</div>
                      </div>
                    </div>
                    {ua.flags?.length > 0 && (
            ...
            {result.brand_impersonation?.length > 0 && (
              <div style={{
                background: '#1a1200', borderRadius: 14,
                border: '1px solid #f59e0b40', padding: '14px 20px',
                display: 'flex', alignItems: 'center', gap: 12,
              }}>
                <span style={{ fontSize: 20 }}>🏦</span>
                <div>
                  <p style={{ fontSize: 11, color: '#f59e0b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, margin: '0 0 6px' }}>
                    Brand Impersonation Detected
                  </p>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {result.brand_impersonation.map((b, i) => (
                      <span key={i} style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: result.image_quality ? '2fr 1fr' : '1fr', gap: 14 }}>
              ...
              <div style={{
                background: '#12121e', borderRadius: 14,
                border: '1px solid #2a2a3e', padding: '16px 18px',
              }}>
                <p style={{ fontSize: 11, color: '#6b7280', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
                  📋 Analysis
                </p>
                <p style={{ fontSize: 13, color: '#c0c0d8', lineHeight: 1.6, margin: 0 }}>
                  {result.explanation}
                </p>
              </div>
            ...
            {/* ROW 6 — OCR Extracted Text (collapsible) */}
            {result.ocr_text && (
              <div style={{
                background: '#12121e', borderRadius: 14,
                ...
                {showOcr && (
                  <div style={{ padding: '0 18px 16px' }}>
                    <div style={{
                      ...
                      border: '1px solid #2a2a3e', padding: '12px 14px',
                    }}>
                      <pre style={{
                        fontSize: 12, color: '#a0a0c0',
            ...
            <div style={{
              background: '#0a1a14', borderRadius: 14,
              border: '1px solid #22c55e40', padding: 20,
            }}>
              <p style={{ fontSize: 11, color: '#22c55e', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 14 }}>
                🛡 Recommended Actions
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
